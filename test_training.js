...
async function assertRejects(promiseFn, expectedText, message) {
    ...
    assert(
        err !== null && err.message.includes(expectedText),
...
assert(
    close(rampedCoherence(15, 1.0, 0.3, 15), 0.3),
    'trial rampLength is clamped to the test level',
);
...
assert(
    ...
    'ramp is monotonically non-increasing',
);
assert(
    descending.every((v) => v >= 0.3 && v <= 1.0),
    'ramp stays within [to, from]',
);
...
const firstWindow = Array.from({ length: 16 }, (_, i) =>
    rampedCoherence(i, 1.0, 0.3, TRAINING_RAMP_LENGTH),
);
assert(
    close(firstWindow[15], 0.3),
    'trial 15 (last of the first criterion window) is at test level',
);
assert(
    firstWindow.slice(TRAINING_RAMP_LENGTH - 1).every((v) => close(v, 0.3)),
...
assert(
    summarizeAdvancementWindow(staleGood).criterionMet === false,
    'earlier successes outside the window do not count',
);
assert(
    close(summarizeAdvancementWindow(staleGood).accuracy, 0),
    'final-window accuracy reflects only the last 16 trials',
);

assert(
    meetsAdvancementCriterion(exactly14) === summarizeAdvancementWindow(exactly14).criterionMet,
    'meetsAdvancementCriterion agrees with the summary it delegates to',
);
assert(
    meetsAdvancementCriterion(Array(8).fill(true), 8, 7) === true,
...
assertThrows(
    () => buildSharedTrainingStages({ ...BASE_SPEC, keyMaps: { mov: {} } }),
    'throws when a key map is missing',
);
assertThrows(
    () => buildSharedTrainingStages({ ...BASE_SPEC, rso: undefined }),
    'throws without rso',
);
assertThrows(
    () => buildSharedTrainingStages({ ...BASE_SPEC, testCoherenceTarget: 0.8 }),
    'throws when testCoherenceTarget is not per-task',
);
assertThrows(
    () => buildSharedTrainingStages({ ...BASE_SPEC, testCoherenceDistractor: undefined }),
    'throws without a distractor test level',
);
assertThrows(
    () => buildSharedTrainingStages({ ...BASE_SPEC, cueCsi: 0 }),
...
assert(
    ...
    'there is no S1 — the untimed key-mapping drill was dropped 2026-08-25',
);
assert(
    !stages.some((s) => s.stage === 'S5'),
    'there is no S5 — the congruent-only stage was dropped 2026-08-25',
);
assert(
    !stages.some((s) => s.stage === 'S7'),
    'S7 (the shared PRP stage) is assembled outside this single-task builder',
);
assert(
    stages.every((s) => s.phase === 'training'),
    'every stage is tagged phase=training',
);
assert(
    stages.every((s) => s.blockConfig.paradigm === 'single-task'),
    'every shared stage is single-task (one task on screen at a time)',
);
assert(
    stages.every((s) => s.blockConfig.keyMaps === KEY_MAPS),
    'key maps pass through unchanged',
);
assert(
    stages.every((s) => s.blockConfig.rso === 'disjoint'),
    'rso passes through unchanged',
);
assert(
    stages.every((s) => s.blockConfig.feedback === true),
    'feedback is on throughout training',
);
assert(
...
assert(
    byStage.S4.blockConfig.congruency.conditions.join() === 'univalent',
    'S4 is still univalent',
);
assert(byStage.S4.blockConfig.coherence.distractor === 0, 'S4 has no distractor');
assert(
    byStage.S4.blockConfig.coherenceRamp.to.mov === 0.8 &&
...
for (const [stage, task, level] of [
    ...
    assert(
        ...
        `${stage} is single-task (sustained, no switching)`,
    );
    ...
    assert(s.blockConfig.cueDuration === 0, `${stage} suppresses cues (cueDuration: 0)`);
    assert(
        s.blockConfig.congruency.conditions.join() === 'congruent,incongruent',
...
assert(
    byStage.S6.blockConfig.congruency.conditions.join() === 'congruent,incongruent',
    'S6 carries conflict',
);
...
assert(
    byStage.S6.blockConfig.levelFactors === undefined,
...
assert(
    stroopByStage.S6.blockConfig.coherence.target === LEVELS,
    'S6 uses the test block coherence table verbatim',
);
assert(
    stroopByStage.S6.blockConfig.levelFactors.target.length === 3,
    'S6 exposes every test coherence level',
);
assert(
    stroopByStage.S2.blockConfig.levelFactors === undefined,
...
assert(
    stroopByStage.S2.blockConfig.blockId === 'stroop_train_S2',
    'blockIdPrefix override applies',
);
...
assert(
    stroopish.every((s) => s.blockConfig.rso === 'identical'),
    'shared-key spec propagates',
);
...
for (const [label, built] of [
    ...
    for (const s of built) {
        ...
        if (!trials) continue;
        assert(
            trials.every((t) => t.meta.t1_task === 'mov' || t.meta.t1_task === 'or'),
            `${label} ${s.stage}: every trial has a task`,
        );
        assert(
            trials.every((t) => t.meta.t2_task === null),
            `${label} ${s.stage}: single-task stages never emit a T2`,
        );
        assert(
            trials.every((t) => t.meta.t1_stim_onset === s.blockConfig.csi),
            `${label} ${s.stage}: stimulus onset is the stage CSI`,
        );
        const univalent = s.blockConfig.coherence.distractor === 0;
        assert(
            trials.every((t) => (t.meta.t1_distractor_dir === null) === univalent),
...
assert(
    guardMessage.includes('acceptFirstResponse: true') &&
        guardMessage.includes('earlyResolve: false'),
...
assert(
    resolveRampTarget({ from: 1.0, to: 0.3 }, 'mov', 'b') === 0.3,
    'a scalar ramp target resolves regardless of task',
);
assert(
    resolveRampTarget({ from: 1.0, to: { mov: 0.3, or: 0.7 } }, 'or', 'b') === 0.7,
...
assert(
    Number.isNaN(rampedCoherence(5, 1.0, undefined, 15)),
...
assert(
    TRAINING_SOA_SCHEDULE_LENGTH === 16,
...
assert(
    scheduledSoa(TRAINING_SOA_SCHEDULE_LENGTH, SOAS) === null,
...
assert(
    schedule.every((v) => SOAS.includes(v)),
    'only real SOA levels are scheduled',
);
...
assert(
    schedule.slice(0, TRAINING_SOA_SCHEDULE_LENGTH).includes(Math.min(...SOAS)),
...
assert(
    single.every((v) => v === 250),
    'a single-level schedule is constant',
);
...
assert(
    crowded.every((v) => [100, 300, 600, 900].includes(v)),
    'no interpolated SOAs are invented',
);
...
assertThrows(
    () => buildParadigmFinalStage({ ...S8_BASE, kind: 'switching', keyMaps: { mov: {} } }),
    'throws when a key map is missing',
);
assertThrows(
    () => buildParadigmFinalStage({ ...S8_BASE, kind: 'switching', rso: undefined }),
    'throws without rso',
);
assertThrows(
    () => buildParadigmFinalStage({ ...S8_BASE, kind: 'switching', coherence: undefined }),
    "throws without the test block's coherence",
);
assertThrows(
    () => buildParadigmFinalStage({ ...S8_BASE, kind: 'switching', csi: undefined }),
    'throws without a test CSI',
);
assertThrows(
    () => buildParadigmFinalStage({ ...S8_BASE, kind: 'switching', csi: -1 }),
    'throws on a negative CSI',
);
assertThrows(
    () => buildParadigmFinalStage({ ...S8_BASE, kind: 'nonsense' }),
    'throws on an unknown kind',
);
assertThrows(
    () => buildParadigmFinalStage({ ...S8_BASE, kind: 'switching' }),
    "kind 'switching' requires a switch rate",
);
assertThrows(
    () => buildParadigmFinalStage({ ...S8_BASE, kind: 'stroop' }),
    "kind 'stroop' requires a target task",
);
assertThrows(
    () => buildParadigmFinalStage({ ...S8_BASE, kind: 'stroop', task: 'both' }),
    "kind 'stroop' rejects a task that is not mov/or",
);
assertThrows(
    () => buildParadigmFinalStage({ ...S8_BASE, kind: 'prp', csi: 0, soaLevels: SOAS }),
    "kind 'prp' requires the condition's T1 task",
);
assertThrows(
    () => buildParadigmFinalStage({ ...S8_BASE, kind: 'prp', csi: 0, t1Task: 'mov' }),
...
assert(
    s8Switch.blockConfig.earlyResolve === true && s8Switch.blockConfig.acceptFirstResponse === true,
...
assert(
    s8Prp.blockConfig.soaSchedule.scheduleLength === undefined,
...
assertDoesNotThrow(
    () => assertValidBlockConfig(s8Prp.blockConfig),
...
assert(
    buildParadigmFinalStage({
        ...
        t1Task: 'mov',
        soaLevels: SOAS,
        soaScheduleLength: 24,
    }).blockConfig.soaSchedule.scheduleLength === 24,
    'schedule length is overridable',
);
...
for (const [label, stage] of [
    ...
    assert(
        trials.every((t) => (t.meta.t2_task !== null) === isPrp),
        `S8 ${label}: a T2 exists iff the stage is the dual-task one`,
    );
    if (isPrp) {
        assert(
            trials.every((t) => t.meta.t1_task === 'or' && t.meta.t2_task === 'mov'),
            'S8 prp: task order is fixed for the condition on every trial',
        );
        assert(
            trials.every((t) => SOAS.includes(t.meta.soa)),
            'S8 prp: SOAs come from the test levels',
        );
    }
}
...
for (const [id, expected] of Object.entries(CP_EXPECTED)) {
    ...
    assert(
        training.every((b) => b.phase === 'training'),
        `${id}: every stage is tagged phase=training`,
    );
    assert(
        training.every((b) => b.blockConfig.blockId === `${expected.prefix}_${b.stage}`),
    ...
    assert(
        training.filter((b) => b.stage !== 'S7').every((b) => b.blockConfig.rso === expected.rso),
        `${id}: every stage but S7 uses the test block's response-set organization`,
    );
    assert(
        training.find((b) => b.stage === 'S7').blockConfig.rso === 'disjoint',
        `${id}: the shared PRP stage S7 is two-handed (rso disjoint) regardless of paradigm`,
    );
    assert(
        training.every((b) => b.blockConfig.keyMaps === expected.keyMaps),
        `${id}: every stage uses the test block's key maps`,
    );
    assert(
        training.every((b) => b.blockConfig.feedback === true),
        `${id}: feedback stays on throughout training`,
    );
    assert(
        testBlocks.every((b) => b.blockConfig.feedback === false),
    ...
    assert(
        testBlocks.every((b, i) => b.blockConfig === testSession[i].blockConfig),
        `${id}: test blockConfigs are the very same objects, not copies`,
    );
    assert(
        testBlocks[0].instructions.endsWith(testSession[0].instructions),
        `${id}: the test block keeps its own instructions, with a preamble prepended`,
    );
    assert(
        testBlocks[0].instructions.includes('Practice is over'),
    ...
    if (expected.s8 === 'prp') {
        ...
        assert(
            s8.blockConfig.soaSchedule.levels === CP_PRP_SOA_LEVELS,
        ...
        assert(
            s8.blockConfig.switchRate === testSession[0].blockConfig.switchRate,
...
for (const id of Object.keys(CP_EXPECTED)) {
    const testSession = CP_TEST_SESSIONS[id];
    assert(
        testSession.length >= 2,
        `${id}: the test session has at least two blocks, so a break falls between them`,
    );
    assert(
        testSession.every((b) => b.blockConfig === testSession[0].blockConfig),
...
function measureCellCount(blockConfig, keys) {
    ...
    for (let i = 1; i < PROBE_LEN; i++) {
        // skip trial 0: transition is 'First'
...
const CP_EXPECTED_CELLS = {
    cp_prp: 6,
    cp_taskswitch: 8,
    cp_taskswitch_asym: 4,
    cp_stroop: 2,
    cp_stroop_crossed: 18,
};
...
for (const [id, keys] of Object.entries(CP_CROSSINGS)) {
    ...
    assert(
        CP_TEST_SESSIONS[id].every((b) => b.numTrials === n),
...
assert(
    70 % CP_EXPECTED_CELLS.cp_stroop_crossed !== 0,
...
for (const id of Object.keys(CP_EXPECTED)) {
    const testSession = CP_TEST_SESSIONS[id];
    assert(
        testSession.length === CP_TEST_BLOCKS_PER_SESSION,
        `${id}: ${CP_TEST_BLOCKS_PER_SESSION} test blocks (found ${testSession.length})`,
    );
    assert(
        typeof testSession[0].instructions === 'string' && testSession[0].instructions.length > 0,
        `${id}: the first block carries the instruction screen`,
    );
    assert(
        testSession.slice(1).every((b) => b.instructions === null),
    ...
    assert(
        testSession.every((b) => b.sequenceSlice === undefined),
...
for (const id of Object.keys(CP_EXPECTED)) {
    for (const condition of ['A', 'B']) {
        const ids = drawSequenceIds(
            `test-pid|${id}|${condition}`,
            CP_SEQUENCE_POOL_SIZE,
            CP_TEST_BLOCKS_PER_SESSION,
        );
...
assert(
    stroopBS2.blockConfig.coherence.target === 0.5,
    'Stroop condition B S2: mov ramps to distractor level (0.5)',
);
assert(
    stroopBS3.blockConfig.coherence.target === 0.8,
    'Stroop condition B S3: or ramps to target level (0.8)',
);
assert(
    stroopBS8.blockConfig.task1 === 'or',
    'Stroop condition B S8: rehearsal task is orientation',
);
assert(
    !/ONE question|which way are the birds/.test(stroopBS8.instructions),
    'Stroop condition B S8 copy: agnostic — does NOT name a single target task',
);
assert(
    stroopBS8.instructions === prpBS8.instructions,
...
assert(
    stroopxBS8.blockConfig.task1 === 'or',
    'Stroop crossed condition B S8: rehearsal task is orientation',
);
assert(
    !/ONE question|which way are the birds/.test(stroopxBS8.instructions),
...
assert(
    tsaBS2.blockConfig.coherence.target === 0.3,
    'Asym switching condition B S2: mov ramps to hard level (0.3)',
);
assert(
    tsaBS3.blockConfig.coherence.target === 0.8,
    'Asym switching condition B S3: or ramps to easy level (0.8)',
);
assert(
    tsaBS8.blockConfig.coherence.target.mov === 0.3 &&
        tsaBS8.blockConfig.coherence.target.or === 0.8,
...
assert(
    appliedPrpBS8.blockConfig.task1 === 'or' || appliedPrpBS8.blockConfig.t1Task === 'or',
    'cpApplySweetPea swaps PRP S8 trial structure to condition B',
);
assert(
    appliedPrpBS8.instructions === prpBS8.instructions,
...
assertThrows(
    () => cpApplySweetPea(CP_TEST_SESSIONS.cp_stroop, 'A', [1, 2, 3, 4]),
    'fewer ids than test blocks is refused — a block would fall back to the JS generator',
);
assertThrows(
    () => cpApplySweetPea(CP_TEST_SESSIONS.cp_stroop, 'A', [1, 2, 3, 4, 5, 6]),
    'more ids than test blocks is refused',
);
assertThrows(
    () => cpApplySweetPea(CP_TEST_SESSIONS.cp_stroop, 'A', [1, 2, 3, 4, 4]),
...
for (const [id, keys] of Object.entries(CP_CROSSINGS)) {
    ...
    assert(
        rows.length === CP_TEST_SESSIONS[id][0].numTrials,
...
for (const id of Object.keys(CP_EXPECTED)) {
    ...
    const readRows = (condition) => {
        const lines = fs
            .readFileSync(cpSequencePath(id, condition, 1), 'utf8')
            .trim()
            .split('\n');
    ...
    assert(
        [...differing].every((col) => CONDITION_LABEL_COLUMNS.has(col)),
    ...
    if (id === 'cp_taskswitch') {
        ...
        assert(
            differing.has('task') || differing.has('target_coh_level'),
...
assert(
    stageOf('cp_taskswitch', 'S2').blockConfig.coherenceRamp.to === 0.8,
    'basic switching ramps S2 to the easy level, not the hard one',
);
assert(
    stageOf('cp_stroop_crossed', 'S2').blockConfig.coherenceRamp.to === 0.7,
...
assert(
    stageOf('cp_taskswitch_asym', 'S2').blockConfig.coherenceRamp.to === 0.8 &&
...
assert(
    stageOf('cp_stroop', 'S3').blockConfig.coherenceRamp.to === 0.5,
...
for (const id of ['cp_taskswitch', 'cp_stroop', 'cp_stroop_crossed', 'cp_taskswitch_asym']) {
    assert(
        stageOf(id, 'S6').blockConfig.coherence === CP_TEST_SESSIONS[id][0].blockConfig.coherence,
        `${id}: S6 uses the test block's coherence table verbatim`,
    );
}
assert(
    stageOf('cp_stroop_crossed', 'S6').blockConfig.levelFactors ===
...
for (const id of Object.keys(CP_EXPECTED)) {
    assert(
        stageOf(id, 'S3a').blockConfig.coherence.distractor > 0,
        `${id}: S3a (Stroop) is bivalent`,
    );
    assert(
        stageOf(id, 'S2').blockConfig.coherence.distractor === 0,
        `${id}: S2 is univalent — bivalence arrives at S3a`,
    );
}
assert(
    CP_TEST_SESSIONS.cp_prp[0].blockConfig.coherence.distractor === 0,
...
for (const [id, expected] of Object.entries(CP_EXPECTED)) {
    ...
    assert(
        /J = left/.test(all) === disjoint,
        `${id}: the second hand's keys appear iff the paradigm uses disjoint keys`,
    );
    assert(/right hand/i.test(all) === disjoint, `${id}: a second hand is named iff there is one`);
    ...
    for (const stage of ['S4', 'S6']) {
        ...
        const cues = new Set(
            (stageDef.demo ? stageDef.demo.segments : [])
    ...
    for (const blockDef of CP_SESSIONS[id]) {
        ...
        assert(
            !/\b(dot|dots|triangle|triangles|circle|circles)\b/i.test(blockDef.instructions),
    ...
    for (const blockDef of CP_SESSIONS[id]) {
        ...
        assert(
            !namesDimension || /FLYING|FACING/i.test(blockDef.instructions),
...
assert(
    s8Texts.every((t) => !/comes FIRST|ONE question|switching between/.test(t)),
...
for (const id of Object.keys(CP_EXPECTED)) {
    ...
    const validKeys = new Set(
        [...Object.values(keyMaps.mov), ...Object.values(keyMaps.or)].map((k) =>
    ...
    for (const stage of CP_TRAINING_STAGES) {
        ...
        assert(
            demo && Array.isArray(demo.segments) && demo.segments.length > 0,
        ...
        for (const seg of demo.segments) {
            ...
            for (const ev of events) {
                ...
                assert(
                    validKeys.has(String(ev.key).toLowerCase()),
                    `${id}/${stage}: demo key '${ev.key}' is one of this paradigm's own keys`,
                );
                assert(
                    DEMO_KEYCAPS[String(ev.key).toLowerCase()] !== undefined,
            ...
            assert(
                (seg.duration || 2400) >= latest + 900 + 400,
    ...
    for (const stage of ['S2', 'S3', 'S3a', 'S3b']) {
        const demo = stageOf(id, stage).demo;
        assert(
            demo.segments.every((seg) => !seg.border),
    ...
    assert(
        testBlocks[0].demo && testBlocks[0].demo.segments.length > 0,
        `${id}: the first test block has a cartoon`,
    );
    assert(
        testBlocks.slice(1).every((b) => !b.demo),
    ...
    assert(
        testText.includes(INSTRUCTION_DEMO_ANCHOR),
        `${id}: the test screen names where its cartoon goes`,
    );
    assert(
        testText.split(INSTRUCTION_DEMO_ANCHOR).length === 2,
    ...
    for (const hasDemo of [true, false]) {
        assert(
            !instructionHtml(testText, hasDemo).includes(INSTRUCTION_DEMO_ANCHOR),
...
for (const id of Object.keys(CP_EXPECTED)) {
    assert(
        stageOf(id, 'S2').demo.segments.every((s) => s.orientation === null && s.movement !== null),
        `${id}/S2: cartoon flies without facing`,
    );
    assert(
        stageOf(id, 'S3').demo.segments.every((s) => s.movement === null && s.orientation !== null),
...
for (const id of Object.keys(CP_EXPECTED)) {
    ...
    assert(
        a.movement === b.movement && a.orientation === b.orientation,
...
assert(
    prpDemo.segments.every((seg) => seg.then),
    'cp_prp/S8: every cartoon segment brings in a second stimulus',
);
assert(
    prpDemo.segments[0].then.at > prpDemo.segments[1].then.at,
    'cp_prp/S8: cartoon SOA descends, matching soaSchedule',
);
assert(
    prpDemo.segments.every((seg) => Array.isArray(seg.then.border) && seg.then.border.length === 2),
...
assert(
    !/harder|difficult|easier/i.test(asymCopy),
    'cp_taskswitch_asym does not tell the participant one task is harder',
);
assert(
    asymCopy === CP_TEST_SESSIONS.cp_taskswitch[0].instructions,
...
const perfRow = (a1, rt1, a2 = null, rt2 = null) => ({ accuracy1: a1, rt1, accuracy2: a2, rt2 });

assert(summarizeBlockPerformance([]) === null, 'no rows means nothing to report');
assert(
    summarizeBlockPerformance([perfRow(null, null)]) === null,
...
const dualPerf = summarizeBlockPerformance([
    perfRow('correct', 500, 'error', 900),
    perfRow('correct', 700, 'correct', 1100),
]);
...
assert(
    !/well done|good|slow|too many/i.test(line),
    'the wording is identical regardless of performance — a varying phrasing would be an evaluative signal, not calibration',
);
assert(
    !/\bms\b/.test(formatBreakSummary(noRtPerf)),
    'the RT clause is dropped when there is no RT',
);
...
async function main() {
    ...
    assert(
        log[0].exclusionCandidate === false,
        'meeting criterion is never an exclusion candidate',
    );
    assert(data.length === 16, 'only the trials actually run are logged');
    assert(
        data.every((r) => r.phase === 'training'),
        'every training row is tagged phase=training',
    );
    assert(
        data.every((r) => r.stage === 'S2'),
        'every training row is tagged with its stage',
    );
    ...
    assert(
        log[0].trialsUsed === TRAINING_CAP,
        `a failing stage runs the full cap (${TRAINING_CAP})`,
    );
    ...
    seResponder = RESPOND_INCORRECT;
    await Session.runSession([trainingBlockDef('S3'), trainingBlockDef('S5')], container, {
        stimulus: 'abstract',
    });
    ...
    await Session.runSession([trainingBlockDef('S2'), laterBlock], container, {
        stimulus: 'abstract',
    });

    assert(
        laterBlock.blockConfig.coherence === laterCoherence,
    ...
    assert(
        testRows.every((r) => r.phase === 'test'),
        'blocks without a phase default to test',
    );
    assert(
        testRows.every((r) => r.stage === null),
        'blocks without a stage default to null',
    );
    ...
    assert(
        settled.every((r) => close(r.t1_target_coherence, r.t1_task === 'mov' ? 0.3 : 0.7)),
    ...
    await Session.runSession([buildParadigmFinalStage(PRP_S8_SPEC)], container, {
        stimulus: 'abstract',
    });
    ...
    data = Session.getData();
    assert(
        log.length === 1 && log[0].stageId === 'S8',
        'the PRP S8 stage is logged as a training stage',
    );
    assert(
        log[0].trialsUsed === 16,
        'both-correct trials reach criterion at the first full window',
    );
    assert(log[0].criterionMet === true, 'criterion met when both responses are correct');
    assert(
        data.every((r) => r.accuracy1 === 'correct' && r.accuracy2 === 'correct'),
        'both responses were extracted and scored on every trial',
    );
    assert(
        data.every((r) => r.paradigm === 'dual-task'),
        'the stage really ran the dual-task paradigm',
    );
    ...
    data = Session.getData();
    assert(
        data.every((r) => r.accuracy1 === 'correct'),
        'T1 was correct on every trial',
    );
    assert(
        data.every((r) => r.accuracy2 === 'error'),
        'T2 was wrong on every trial',
    );
    assert(
        log[0].trialsUsed === TRAINING_CAP,
    ...
    data = Session.getData();
    assert(
        data.every((r) => r.accuracy1 === 'error'),
        "the first press in T1's key set is T1's response, wrong or not (strict scoring)",
    );
    assert(
        data.every((r) => r.rt1 !== null && r.rt1 > 0),
        'that wrong press still yields an RT',
    );
    assert(
        data.every((r) => r.accuracy2 === 'correct'),
        'a wrong press on T1 no longer costs the T2 response',
    );
    assert(
        data.every((r) => r.rt2 !== null && r.rt2 > 0),
        'and T2 keeps a usable RT',
    );
    assert(
        Session.getTrainingLog()[0].criterionMet === false,
    ...
    assert(
        data.every((r) => r.rt1 > r.rt2),
        'T1 responded later than T2 — the reversal is preserved, not normalized away',
    );
    assert(
        data.every((r) => r.responseOrder === 'T2-first'),
        'and it is labelled as a reversal',
    );
    assert(
        log[0].criterionMet === true && log[0].trialsUsed === 16,
    ...
    assert(
        buildParadigmFinalStage({ ...S8_BASE, kind: 'switching', switchRate: 50 })
            .advancementThreshold === undefined,
        "kind 'switching' inherits the shared default",
    );
    assert(
        buildParadigmFinalStage({ ...S8_BASE, kind: 'stroop', task: 'mov' })
            .advancementThreshold === undefined,
        "kind 'stroop' inherits the shared default",
    );
    assert(
        buildSharedTrainingStages(BASE_SPEC).every((s) => s.advancementThreshold === undefined),
    ...
    const singleScripted = (correctPerWindow) => () => {
        ...
        return {
            keyPresses: [
                { eventType: 'keydown', key: isCorrect ? 'd' : 'a', time: 900, isCorrect },
            ],
        };
    };
    ...
    assert(
        log[0].advancementThreshold === 14,
        'and it was scored against the shared 14/16 default',
    );
    ...
    for (const [label, stage, responder, expectMet] of [
        ...
        assert(
            stoppedEarly === summary.criterionMet,
    ...
    assert(
        scheduled[0] === 600,
        'the stage opens at the longest SOA (T1 finishes before T2 appears)',
    );
    assert(
        scheduled[TRAINING_SOA_SCHEDULE_LENGTH - 1] === 100,
        'the schedule ends at the shortest SOA',
    );
    assert(
        scheduled.every((v, i) => i === 0 || v <= scheduled[i - 1]),
        'the scheduled SOAs are monotonically non-increasing',
    );
    assert(
        new Set(scheduled).size === 3,
        'all three SOA levels appear inside the criterion window',
    );
    assert(
        data.every((r) => [100, 300, 600].includes(r.soa)),
        'every trial uses a real SOA level',
    );
    assert(
        data.every((r) => r.t2_stim_onset === r.soa),
    ...
    assert(
        afterSchedule.length === TRAINING_CAP - TRAINING_SOA_SCHEDULE_LENGTH,
        'the rest of the cap ran after the schedule',
    );
    assert(
        !afterSchedule.every((v, i) => i === 0 || v <= afterSchedule[i - 1]) ||
    ...
    const twoTestBlocks = [
        {
            blockConfig: { ...TRAINING_BLOCK_CONFIG, blockId: 'test_a' },
            numTrials: 4,
            instructions: 'A',
        },
        {
            blockConfig: { ...TRAINING_BLOCK_CONFIG, blockId: 'test_b' },
            numTrials: 4,
            instructions: 'B',
        },
    ];
    ...
    assert(
        ...
        'it reports the accuracy of the block just finished',
    );
    assert(/ms per answer/.test(breakScreens[0]), 'and a mean RT');
    assert(
        screens[screens.length - 1].includes('Session complete'),
    ...
    await Session.runSession(
        [
            ...
            trainingBlockDef('S3'),
            {
                blockConfig: { ...TRAINING_BLOCK_CONFIG, blockId: 'test_after' },
                numTrials: 4,
                instructions: 'T',
            },
        ],
    ...
    assert(
        screens.filter((text) => text.includes('Since the last break')).length === 0,
        'no break summary is shown while the session is still in training',
    );
    assert(
        screens.filter((text) => /Block complete\./.test(text)).length === 0,
    ...
    await Session.runSession(
        [
            ...
            {
                blockConfig: { ...TRAINING_BLOCK_CONFIG, blockId: 'test_1' },
                numTrials: 4,
                instructions: 'X',
            },
            {
                blockConfig: { ...TRAINING_BLOCK_CONFIG, blockId: 'test_2' },
                numTrials: 4,
                instructions: 'Y',
            },
        ],
    ...
    breakScreens = screensOf(screenContainer).filter((text) =>
        text.includes('Since the last break'),
    );
    ...
    await Session.runSession(
        [
            ...
            trainingBlockDef('S3'),
            {
                blockConfig: { ...TRAINING_BLOCK_CONFIG, blockId: 'seam_test' },
                numTrials: 4,
                phase: 'test',
                instructions: 'T',
            },
    ...
    const seamBreaks = screens.filter(
        (t) => t.includes('Training complete') && t.includes('test blocks begin'),
    );
    ...
    assert(
        /Press Enter/.test(seamBreaks[0]),
        'and advances on a deliberate Enter press, not any key',
    );
    assert(!/press any key/i.test(seamBreaks[0]), 'the break never invites an any-key dismissal');
    assert(
        !seamBreaks[0].includes('Since the last break'),
        'the seam break carries no performance summary — no test data has accrued yet',
    );
    assert(
        screens.filter((t) => t.includes('Since the last break')).length === 0,
    ...
    screenContainer = makeElement();
    await withFastClock(() =>
        Session.runSession(CP_SESSIONS.cp_stroop, screenContainer, { stimulus: 'abstract' }),
    );
    ...
    assert(
        breakScreens.length === expectedBreaks,
    ...
    assert(
        testBlockOrders.length === CP_TEST_BLOCKS_PER_SESSION,
        `the test phase ran as ${CP_TEST_BLOCKS_PER_SESSION} separate blocks`,
    );
    assert(
        testBlockOrders.every((o) => cpTestRows.filter((r) => r.blockOrder === o).length === 96),
        'each test block ran its full 96 trials',
    );
    assert(data.filter((r) => r.phase === 'training').length > 0, 'training really ran too');
    assert(
        screens.filter((t) => /Block complete\./.test(t)).length === expectedBreaks,
    ...
    const poolConfigA = {
        ...TRAINING_BLOCK_CONFIG,
        blockId: 'pooled',
        sequenceSource: 'sequences/x_A_s001.csv',
        sequenceId: 1,
    };
    const poolConfigB = {
        ...TRAINING_BLOCK_CONFIG,
        blockId: 'pooled',
        sequenceSource: 'sequences/x_A_s002.csv',
        sequenceId: 2,
    };
    seResponder = RESPOND_CORRECT;
    await Session.runSession(
    ...
    assert(
        block1.length === 4 && block2.length === 4,
        'each block runs its own whole 4-row CSV — no file is split',
    );
    assert(
        block1.every((r) => r.t1_target_dir === 180) && block2.every((r) => r.t1_target_dir === 0),
    ...
    assert(
        block1.every((r) => r.sequenceId === 1) && block2.every((r) => r.sequenceId === 2),
    ...
    assert(
        Session.getData().every((r) => r.sequenceId === null),
